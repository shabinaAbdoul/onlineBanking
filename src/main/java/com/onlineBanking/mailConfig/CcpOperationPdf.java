...
public class CcpOperationPdf {
	
		 private List<CcpOperation> ccpOperations;
		 private UserDetails userDetails;
		    public CcpOperationPdf(List<CcpOperation> ccpOperations, UserDetails userDetails) {
			super();
			this.ccpOperations = ccpOperations;
		    ...
		    private void writeTableData(PdfPTable table) {
		        for (CcpOperation ccpOp : ccpOperations) {
		        	SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd/MM/yyyy");
		    		String myDateString = simpleDateFormat.format(ccpOp.getCreationDateTime());
		            table.addCell(myDateString);
		            table.addCell(ccpOp.getDescription());
		            table.addCell(ccpOp.getStatus().toString());
		            table.addCell(ccpOp.getType());
		            table.addCell(ccpOp.getAmount().toString());
		        }
		    }
		    ...
		    public void export(HttpServletResponse response) throws DocumentException, IOException {
	    		...
	    		String month=ccpOperations.get(0).getCreationDateTime().toString().split("-")[1];
		        ...
		        // p1.add(new Phrase("au 01/"+(Integer.parseInt(ccpOperations.get(0).getCreationDateTime().toString().split("-")[1])+01)+"/2022", fontSmall));
		        document.add(p1);
		        ...
		        cell.setPhrase(new Phrase("CCP n° "+ccpOperations.get(0).getCcpAccount().getId(), fontM));
		         table1.addCell(cell);
		        cell.setPhrase(new Phrase("+ "+ccpOperations.get(0).getCcpAccount().getBalance(), fontM));
		        table1.addCell(cell);
		        ...
		        cell.setPhrase(new Phrase("+ "+userDetails.getLivretA().getBalance(), fontM));
		         ...
		         document.add(table1);
		         document.add(Chunk.NEWLINE);
		         Paragraph p2 = new Paragraph("Comptes courant postal ", font);
			        p2.setAlignment(Paragraph.ALIGN_LEFT);
			        p2.add(new Phrase("n° "+ccpOperations.get(0).getCcpAccount().getId(), fontSmall));
			        document.add(p2);	
			        Paragraph p3 = new Paragraph("IBAN: "+ccpOperations.get(0).getCcpAccount().getNumIban(), fontSmall);  
			        document.add(p3);
